...
S_ONE = Summary("jj_summary", "Time spent counting birds")
S_TWO = Summary("jj_summartwo", "Time spent counting more birds, and differently")
C_ONE = Counter("jj_counter", "Counter of stuff (and things too)")
...
@S_ONE.time()  # <- decorators don't seem to record?
@api.get("/count_birds")
def count_birds(request) -> int:
    with S_TWO.time():  # <- context managers are fine
        C_ONE.inc()
