...
def process_task(_, name, value):
    ...
    time.sleep(random.randint(1, 5))
    update_task_status(task_id, "SUCCESS", 100, f"{name} completed in {time.perf_counter() - start_time} seconds")

    return {"status": "completed", "time_taken": time.time() - start_time}

def update_task_status(task_id, status, progress, result=None):
    redis_client.hmset(task_id, {"status": status, "progress": progress, "result": result if result else ""})
