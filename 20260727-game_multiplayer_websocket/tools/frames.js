...
// few with their update/render split so the cost can be attributed.
//
...
for (const phase of ['build', 'battle']) {
  ...
  const snap = await page.evalIn('JSON.stringify(__game.perf.snapshot(__game.sceneCtl.renderer))');
